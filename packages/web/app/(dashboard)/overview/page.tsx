...
export default async function OverviewPage() {
  const enabledPipelines = PIPELINE_LIST.filter((p) => p.enabled).length;
  const totalPipelines = PIPELINE_LIST.length;
  const runs = isSupabaseConfigured() ? await listRuns({ limit: 1 }) : [];
  const totalRuns = isSupabaseConfigured() ? (await listRuns({ limit: 1000 })).length : null;
    ...
    <>
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          label="Agents Mastra"
          value={9}
          hint="incluye assistant + 8 pedagógicos"
          icon={<BotIcon />}
          href="/agents"
        />
        <StatCard
          label="Pipelines"
          value={`${enabledPipelines}/${totalPipelines}`}
          hint="activos / totales"
          icon={<PlayIcon />}
          href="/pipelines"
        />
        <StatCard
          label="Runs guardados"
          value={totalRuns ?? "—"}
          hint={totalRuns === null ? "Supabase no configurado" : "en library"}
          delta={runs.length > 0 ? { value: "Recientes", tone: "positive" } : undefined}
          icon={<ArchiveIcon />}
          href="/library"
        />
        <StatCard
        ...
        <article className="flex flex-col gap-3 rounded-xl border bg-card p-5 shadow-xs">
          ...
          <div className="flex flex-col gap-2">
            ...
            <ShortcutRow
              ...
              hint="exerciseGenerator · debuggingTutor"
              href="/pipelines"
            />
            <ShortcutRow
              icon={<ArchiveIcon />}
              label="Library de runs"
              hint="Historial guardado"
              href="/library"
            />
            <ShortcutRow
              icon={<BotIcon />}
